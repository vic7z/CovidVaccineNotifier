...
public class Notify {
  ...
  public void sendSms(User user) {
    ...
    if (!user.getAvailableCenters().isEmpty()) {
      String message = "Hi " + user.getUserName() + ". \r\n" + user.getAvailableCenters().size()
          + " vaccination centers available for " +
  ...
  private int getDosage(User user) {
    ...
    if (user.getDosageType() == 1) {
      sum = user.getAvailableCenters().get(0).getSessions().stream()
          .mapToInt(SessionList::getAvailable_capacity_dose1).sum();
    } else {
      sum = user.getAvailableCenters().get(0).getSessions().stream()
          .mapToInt(SessionList::getAvailable_capacity_dose2).sum();
    }
    return sum;
  }
